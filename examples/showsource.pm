...
sub showSource {
    ...
    $color_Keys = { 'Variable_Scalar'   => 'red',
    ...
    while ( my ( $type, $style ) = each %{$color_Keys} ) {
        $formatter->set_format( $type,
                                [  qq|<span style="color:$style;">|, '</span>'
                                ]
        );
    }
    $perldoc_Keys = { 'Builtin_Operator' => 'blue',
    ...
    while ( my ( $type, $style ) = each %{$perldoc_Keys} ) {
        $formatter->set_format(
            $type,
            [  qq|<a onclick="window.open('http://perldoc.perl.org/search.html?q='+this.innerHTML)" style="color:$style">|,
               "</a>"
            ]
        );
    }
